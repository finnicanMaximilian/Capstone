...
<img src="pokerMenu.png" alt="Figure 1" align="middle"> 
<p align="center"><strong>Fig 1</strong></p>
<img src="pokerRules.png" alt="Figure 2" align="middle">
<p align="center"><strong>Fig 2</strong></p>

## Objectives
<p align="left">

&emsp;Poker Fanatic is meant to teach someone how to become good at poker. What I wanted to have available for the user is easy ways to start the game, adjustable versions of the game so the user can start at their level of expertise, and feedback to show them what they got based off the combination of cards in their hand. I wanted an intuitive design so that  when a user runs the game they immediately know what to do. To produce this type of design the user is prompted with three buttons reading “easy”, “medium”, and “hard” which are also colored green yellow and red for visual ques. When clicked the rules of poker are displayed and a button that says “deal out cards”. Hopefully after reading the graphic the user understands the basics of the game. Then the user can deal out the cards and begin the game. Now the user has a bunch of cards and may not know what to do. A map of poker hand ranking chart is displayed to help aid the user’s decision for what cards to give away. Once you have given back your cards the user is prompted to "flip cards". Points will be evaluated for your final hand and printed after you flip the opponents hand.  This was implemented to help see the amount of points that their hand generated.  
...
&emsp; I structured my classes based off the software engineering prinicpal "encapsulate what varies", by creating a deck class this allowed me to assemble and create all the cards simultaneously. The fields and methods are as specific to that class as possible although improvement can always be accomplished. After completing the project i could see how programming a "dealer.java" class would clean up alot of the code inside my GUI to help futhur satisfy the principal. Removing the method InitiateGiveBack(int) is a perfect example of code that could be removed from the PokerTwoGui class because it interacts with the player and deck, so not the actual Gui, therefore it should be encapsulated into a separate class. Globals such as card#Name, card1Clicked, and card1 are all neccisary to capture the users click and then to give information back into initiateGiveBack.
